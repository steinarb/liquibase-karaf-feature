...
/NOTE! Version 4.15.0 is the first release of the liquibase karaf feature in nearly three years/ See [[Notes for upgrading from liquibase 3.x to 4.x]] if you come from version 3.8.0 of this feature.
...
** Notes for upgrading from liquibase 3.x to 4.x
...
But to make liquibase usage work, the bundle that instanciates the Liquibase classes and loads and parses the changelog files, needs to do two things:
 1. Add a Require-Capabilities header that requires all capabilities provided by the liquibase-core bundle
 2. Extract the XSD file(s) for the changelog schema version(s) used and add them to the bundle doing the parsing
