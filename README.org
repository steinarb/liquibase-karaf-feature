...
|------------------------+----------+-------------------+-------------------------+--------------------------------------------------------------|
| <2024-07-03 Wed 22:07> | 4.28.0.1 |            4.28.0 |                         | Bugfix for 4.28.0 see also [[Current problems under OSGi]]       |
...
To install this feature:
 1. start karaf and give the following commands to the karaf console:
    #+BEGIN_EXAMPLE
      feature:repo-add mvn:no.priv.bang.karaf/liquibase-core-karaf/4.28.0.1/xml/features
...
<features xmlns="http://karaf.apache.org/xmlns/features/v1.4.0" name="ukelonn.bundle.db.liquibase">
    <repository>mvn:no.priv.bang.karaf/liquibase-core-karaf/4.28.0.1/xml/features</repository>
...
Apart from the issues worked around in the previous section I see the following problems:
 1. liquibase 4.28.0 is missing an SPI service start (https://github.com/liquibase/liquibase/issues/6040 ) so to use 4.28.0 under OSGi it is neccessary to manually start the missing service, like e.g so:
    #+begin_src java
      public class SomeClass {
          ...
          public CommandResults applyLiquibaseChangelist(Connection connect, String changelistClasspathResource, ClassLoader classLoader) throws LiquibaseException {
              try (var database = findCorrectDatabaseImplementation(connect)) {
                  ...
                  return Scope.child(scopeObjectsWithClassPathResourceAccessor(classLoader), () -> ...);
              }
          }
    ...
    #+end_src
    (/Note/: the [[https://github.com/steinarb/liquibase-karaf-feature/blob/master/liquibase-libraries/karaf.liquibase.runner/src/main/java/no/priv/bang/karaf/liquibase/runner/LiquibaseClassPathChangeLogRunner.java#L42C14-L42C47][LiquibaseClassPathChangeLogRunner]] new with the 4.28.0.1 of liquibase-karaf-feature [[https://github.com/steinarb/liquibase-karaf-feature/blob/master/liquibase-libraries/karaf.liquibase.runner/src/main/java/no/priv/bang/karaf/liquibase/runner/LiquibaseClassPathChangeLogRunner.java#L42C14-L42C47][implements this workaround]] (which will be removed once the issue is fixed in a liquibase release))
